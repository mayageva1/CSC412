#!/usr/bin/env bash
# Usage: bash Scripts/script05v1.sh <data_folder> <output_folder>
# Works with Program v2 or v3. Builds first. Strict v1 format only.

set -euo pipefail

# --- build first (prof request: call with bash) ---
...
# --- prepare output folder (empty it) ---
mkdir -p "$OUT"
...
# Optional debug: run with DEBUG=1 to echo each step
DEBUG="${DEBUG:-0}"
