	...
	*height = (unsigned short)(((unsigned int)head[14]&0xFF) | (unsigned int)head[15]*256);
	...
	if (*theType == RGBA32_RASTER)
	{
		...
		if(!(head[17]&0x20))
		{
			...
			{
				unsigned char* rowPtr = data + (*height-i-1) * *width * 4;
	...
	else
	{
		...
		if(!(head[17]&0x20))
		{
			for(int i = 0; i < *height; i++)
			{
				unsigned char* rowPtr = data + (*height-i-1)* *width;
				fread( rowPtr, sizeof(char), *width, tga_in ) ;
			}
		}
		else
		{
...
//----------------------------------------------------------------------*/ 
int writeTGA(char* filePath, unsigned char* raster,
