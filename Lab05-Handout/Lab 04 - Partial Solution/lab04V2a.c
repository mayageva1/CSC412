...
void childProcess(int index, char* commandStr)
{
	//	In this version, the chikd process just prints out intent
	printf("Child Process #%d, with pid %d, will execute the command:\n\t%s\n",
			index, getpid(), commandStr);
			
	//	The child process must exit at the end of this function.  Otherwise, it would
	//	return to the main loop and create unwanted grandchild processes.
	exit(0);
}
