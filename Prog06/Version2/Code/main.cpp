...
int main(int argc, const char** argv)
{
	...
	//Aapplication-level initialization
	initializeApplication();

	//	Version 2: create persistent worker threads
	...
	//	Now we enter the main loop of the program
	glutMainLoop();
...
void cleanupAndQuit(void)
{
    ...
    // wake up any threads that might be waiting on startWork
    for (int t = 0; t < numThreads; ++t)
    {
        if (threadInfoArray != nullptr)
        {
            threadInfoArray[t].startWork = true;
        }
    }
...
void* threadFunc(void* arg)
{
    ...
    while (!quitFlag)
    {
        // wait until main says to work
        while (!info->startWork && !quitFlag)
        {
            usleep(1000);   // 1 ms
        }
        ...
        // do our part of this generation
        computeRange(info->startRow, info->endRow);

        // mark this thread done for this generation
        info->startWork = false;
        info->finished  = true;
    }
...
{
    //  all threads to start working on their row ranges
    for (int t = 0; t < numThreads; ++t)
    {
        threadInfoArray[t].finished  = false;
        threadInfoArray[t].startWork = true;
    }

    // wait until all threads report they are finished
    bool allDone = false;
    while (!allDone)
    {
        allDone = true;
    ...
    // all threads have filled nextGrid -> safe to swap
    swapGrids();
