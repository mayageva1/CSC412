...
{
	//programName <width> <height> <numThreads>
    if (argc != 4)
    {
...
void oneGeneration(void)
{
    ...
    // create one short-lived thread per range
    for (int t = 0; t < numThreads; ++t)
    {
        // pass the address of the corresponding ThreadInfo
        int rc = pthread_create(&threadHandles[t],
                                nullptr,
                                threadFunc,
                                &threadInfoArray[t]);
        if (rc != 0)
