...
// thread information
int numThreads = 0;
int maxNumThreads = 0;   // just to satisfy the extern in gl_frontEnd.cpp
...
// Version 3
bool quitFlag = false;// tell worker threads to exit
...
int main(int argc, const char** argv)
{
    ...
    //	Version 3: create persistent worker threads
	...
	//	enter the main loop of the program and to a large extend
	//	"lose control" over its execution.  The callback functions that 
	// set up earlier will be called when the corresponding event
	//	occurs
	glutMainLoop();
...
void cleanupAndQuit(void)
{
    ...
    quitFlag     = true;
    workAvailable = false;
    pthread_cond_broadcast(&workCond);  // wake any threads waiting for work
	...
	//	Free allocated resource before leaving
...
void* threadFunc(void* arg)
{
    ...
    while (true)
    {
        // Wait for work for this generation or quit signal
        pthread_mutex_lock(&workMutex);
        while (!workAvailable && !quitFlag)
        {
        ...
        // computation on our row range
        computeRange(info->startRow, info->endRow);

        // Barrier: notify that this thread is done with this generation
        pthread_mutex_lock(&workMutex);
        ...
        {
            // Last thread: swap grids and signal main that generation is done
            swapGrids();
            workAvailable = false; // no more work until main starts next gen
            pthread_cond_signal(&doneCond);  // wake main thread
        }

        pthread_mutex_unlock(&workMutex);
        // loop back and wait for the next generation
    }
...
void oneGeneration(void)
{
    ...
    {
        usleep(generationSleepMilliSec * 1000); // ms
    }
}
