...
int main(int argc, char* argv[]){
    ...
    char* keyString = argv[8];
    char* outputFilePath = argv[9];
    ...
    for(int j=0; j<numPaths; j++){
        ...
        while(fgets(lineBuffer, sizeof(lineBuffer), weatherFile)) {
            ...
            // handle comment
        // find the comment part (if any)
        // char lineBuffer[256];
        // fgets(lineBuffer, sizeof(lineBuffer), weatherFile); // read the rest of the line
        // currentRecord.comment = getComment(lineBuffer);

            //check if the record is within the binning region or if it does not contain the key string
            if(currentRecord.longitude < minLong || currentRecord.longitude > maxLong ||
                currentRecord.latitude < minLat || currentRecord.latitude > maxLat || (keyString && !strstr(currentRecord.comment, keyString))) {
...
//gcc -Wall -std=c11 Programs/Version-2/prog02_2.c -o prog02_2
//./prog02_2 data3/dflist.txt 3.5 143.5 4 -20.0 70.0 6 storm output2.txt