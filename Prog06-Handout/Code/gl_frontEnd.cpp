...
//  gl_frontEnd.cpp
//
//  Created by Jean-Yves Hervé on 2025-10-24.
//
...
void myWindowResizeFunc(int w, int h);
void displayTextualInfo(const char* infoStr, int x, int y, int isLarge);
void myMouseHandlerFunc(int b, int s, int x, int y);
void myGridPaneMouseHandlerFunc(int b, int s, int x, int y);
void myStatePaneMouseHandlerFunc(int b, int s, int x, int y);
void myKeyHandlerFunc(unsigned char c, int x, int y);
...
extern Rules currentRule;
extern bool useColorMode;
extern int generationSleepMilliSec;
...
//	Note that I don't use a scoped enum type here because glut passes menu
//	selections as int values and casting is cumbersome.
...
const GLfloat cellColor[NUM_COLORS][4] = {	{0.f, 0.f, 0.f, 1.f},	//	BLACK_COL
...
//---------------------------------------------
#pragma mark -
#pragma mark glut-related variables
//---------------------------------------------
#endif
...
void displayTextualInfo(const char* infoStr, int xPos, int yPos, int isLarge)
{
    ...
    //-----------------------------------------------
    unsigned short infoLn = (unsigned short) strlen(infoStr);
...
void drawState(unsigned short numLiveThreads)
{
	...
	snprintf(infoStr, 20, "Live Threads: %d", numLiveThreads);
...
//
void myMouseHandlerFunc(int button, int state, int x, int y)
{
	(void) button;
	(void) state;
	(void) x;
...
//
void myGridPaneMouseHandlerFunc(int button, int state, int x, int y)
{
	(void) state;
	(void) x;
	(void) y;
...
void myStatePaneMouseHandlerFunc(int button, int state, int x, int y)
{
	(void) state;
	(void) x;
	(void) y;
	...
	switch (c)
	{
		...
		case ' ':
			...
			break;
				...
				//	'+' --> increase simulation speed (less sleep)
		case '+':
			if (generationSleepMilliSec > 0)
            {
                generationSleepMilliSec -= 10;
                if (generationSleepMilliSec < 0)
                    generationSleepMilliSec = 0;
            }
			break;

		//	'-' --> reduce simulation speed (more sleep)
		case '-':
            generationSleepMilliSec += 10;
			break;
		...
		case '1':
			currentRule = Rules::GAME_OF_LIFE_RULE;
			break;

		//	'2' --> apply Rule 2 (Coral: B3_S45678)
		case '2':
			currentRule = Rules::CORAL_GROWTH_RULE;
			break;

		//	'3' --> apply Rule 3 (Amoeba: B357/S1358)
		case '3':
			currentRule = Rules::AMOEBA_RULE;
			break;

		//	'4' --> apply Rule 4 (Maze: B3/S12345)
		case '4':
			currentRule = Rules::MAZE_RULE;
			break;

		//  '5' --> apply Rule 5 (HighLife: B36/S23)
        case '5':
            currentRule = Rules::HIGH_LIFE_RULE;
            break;
...
void initializeFrontEnd(int argc, const char** argv,
						void (*gridDisplayCB)(void),
						void (*stateDisplayCB)(void),
						void (*timerCB)(int))
{
	//	Initialize glut and create a new window
	//	The const casting is required because glut is based on
	//	old (pre-const) C  ¯\_(ツ)_/¯
	glutInit(&argc, const_cast<char**>(argv));
	glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA);

	glutInitWindowSize(WINDOW_WIDTH, WINDOW_HEIGHT);
	glutInitWindowPosition(INIT_WIN_X, INIT_WIN_Y);
	gMainWindow = glutCreateWindow("Programming Assignment 05 -- Cellular Automata -- CSC 412 - Fall 2025");
	glClearColor(0.2f, 0.2f, 0.2f, 1.f);
	
	//	set up the callbacks for the main window
	glutDisplayFunc(myDisplayFunc);
	glutReshapeFunc(myWindowResizeFunc);
	glutMouseFunc(myMouseHandlerFunc);
	...
	glutKeyboardFunc(myKeyHandlerFunc);
	glutMouseFunc(myGridPaneMouseHandlerFunc);
