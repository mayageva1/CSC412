...
// -------------------------------------------------------------------
// Dimensions formatting helper for the 'dimensions' application.
...
char* formatDimensions(const RasterImage* img, bool verbose, DimMode mode) {
    ...
    if (verbose) {
        switch (mode) {
            case DM_WIDTH:
                snprintf(buf, sizeof(buf), "width: %hu", img->width);
                break;
            case DM_HEIGHT:
                snprintf(buf, sizeof(buf), "height: %hu", img->height);
                break;
            case DM_BOTH:
            default:
                snprintf(buf, sizeof(buf), "width: %hu, height: %hu", img->width, img->height);
                break;
        }
    } else {
        switch (mode) {
            case DM_WIDTH:
                snprintf(buf, sizeof(buf), "%hu", img->width);
                break;
            case DM_HEIGHT:
                snprintf(buf, sizeof(buf), "%hu", img->height);
                break;
            case DM_BOTH:
            default:
                snprintf(buf, sizeof(buf), "%hu %hu", img->width, img->height);
                break;
        }
    }
...
int compareImagesExact(const RasterImage* a, const RasterImage* b) {
    if (!a || !b || !a->raster || !b->raster) return 0;
    if (a->width  != b->width)  return 0;
    if (a->height != b->height) return 0;
    if (a->type   != b->type)   return 0;
    if (a->bytesPerPixel != b->bytesPerPixel) return 0;
    if (a->bytesPerRow   != b->bytesPerRow)   return 0;

    size_t total = (size_t)a->bytesPerRow * a->height;
    return (memcmp(a->raster, b->raster, total) == 0) ? 1 : 0;
}
